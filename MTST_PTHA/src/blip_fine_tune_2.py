...
def clip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    for epoch in range(num_epochs):
        ...
        for idx, (reference_images, target_images, captions,text_revs) in enumerate(train_bar):
            ...
            text_revs = [txt_processors["eval"](text_rev) for text_rev in text_revs]

            blip_model.train()
            # Extract the features, compute the logits and the loss
            with torch.cuda.amp.autocast():
                loss_dict = blip_model({"image":reference_images, "target":target_images, "text_input":captions,"text_rev":text_revs})
                loss = 0.
...
def clip_finetune_cirr(num_epochs: int, blip_model_name: str, backbone: str, learning_rate: float, batch_size: int,
    ...
    """
    rtc_weights = kwargs['loss_t2t']
    align_weights = kwargs['loss_p2p']
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--loss_p2p", default=0.4, type=float)
    parser.add_argument("--loss_t2t", default=0.4, type=float)
    ...
    training_hyper_params = {
        ...
        "loss_t2t": args.loss_t2t,
        "loss_p2p": args.loss_p2p,
